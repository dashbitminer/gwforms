...
var age= Math.abs(age_dt.getUTCFullYear() - 1970);
  ...
  function ver6(){
    ...
    var tipo= document.getElementById("tipoPerfil").value;
    if (tipo=="1"){
    div.classList.remove("d-none");
  ...
  checkboxes.forEach(function(checkbox) {
    switch (checkbox.value) {
      case "1":
       ...
       break;
      case "2":
        clubtext = "Mentoría entre compañeros";
        break;
      case "3":
        clubtext = "Club de relajación o prácticas";
        break;
    }
      if (checkbox.checked) {
...
function guardar(event){
 ...
 if (tipo=="1"){
  ...
  xhr.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
          ...
          if(respuesta=="1"){
              ...
              window.location.href = "/educacion/ny/esp";
          }else if (respuesta=="-1"){
  ...
  function mostrarclub(tipo)
  {
    if (tipo == "2")
    {
      document.getElementById("club").classList.add("d-none");
    }
    else 
    {
      document.getElementById("club").classList.remove("d-none");
    }
  }