 ...
 if (!form.checkValidity() || invalidElements.length > 0) {
  ...
  Swal.fire({
    ...
    icon: 'success'
          ...
          window.location.href = "/raices/preinscripcion";
        });
      } else {
        Swal.fire({
          icon: 'error',
  ...
  xhr.open("POST", "/raices/preinscripcion/post", true);
  xhr.send(formData);
}
...
function comprobarfecha() {
  ...
  if(edad < 18 && edad >= 15){
Swal.fire({
  text: 'Agradecemos tu interés en participar en el proyecto Raíces Centroamericanas. Al ser una persona menor de edad, solicitamos que tu responsable familiar se contacte directamente con nuestra organización al correo mgmoreno@glasswing.org para compartirles la información necesaria.',
icon: 'info',
customClass: {
  ...
  }else if(edad < 15 || edad > 22){    
Swal.fire({
  text: ' Agradecemos tu interés en participar en el proyecto Raíces Centroamericanas. Lastimosamente el perfil que estamos buscando para esta actividad es de jóvenes de 15 a 22 años. Sin embargo, te invitamos a estar pendiente de nuestras redes sociales en Instagram: Glasswing El Salvador (glasswingsv) y Parque Cuscatlán (miparquecuscatlán) y en Facebook: Glasswing El Salvador para que conozcas otras actividades en las que nos encantaría contar con tu participación.',
icon: 'info',
customClass: {
  content: 'text-justify'
}
