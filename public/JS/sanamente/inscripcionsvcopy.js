...
if(select.value==="1"){    
  labelOutput.innerHTML = "Escribe el número documento unico de identidad (DUI)*"
...
function FSector(paisSeleccionado) {
...
function actualizarmun(paisSeleccionado) {
  ...
  xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  xhr.send("depsede=" + paisSeleccionado +"&area="+area.value);
}
...
function actualizarsede(paisSeleccionado) {
  ...
  xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  xhr.send("munsede=" + paisSeleccionado +"&area="+area.value);
}
